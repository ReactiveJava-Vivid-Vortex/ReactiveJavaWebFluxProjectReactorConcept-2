# Reactive Error Handling — Project Reactor
...
|---|---|---|
| `onErrorReturn` | Error | Return fallback value |
| `onErrorResume` | Error | Switch to another Publisher |
| `onErrorMap` | Error | Transform exception |
| `onErrorComplete` | Error | Convert error into complete |
| `onErrorContinue` | Element-processing error | Skip failing element and continue where supported |
| `doOnError` | Error | Observe/log error; error continues |
| `doFinally` | Complete/Error/Cancel | Execute final side effect |
| `retry` | Error | Retry failed sequence |
| `retryWhen` | Error | Controlled retry/backoff |
| `defaultIfEmpty` | Empty | Return fallback value |
| `switchIfEmpty` | Empty | Switch to another Publisher |
| `timeout` | Slow operation | Turn timeout into error or fallback |
| `using` | Resource lifecycle | Ensure resource cleanup |
| `materialize` | Signals | Convert signals into data |
